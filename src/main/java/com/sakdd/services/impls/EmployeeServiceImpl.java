...
public class EmployeeServiceImpl implements EmployeeService {
		...
		try {
			...
			if (!employees.isEmpty()) {
				for (int i = 0; i < employees.size(); i++) {
					...
					employeeVO.setHomeAddress(employee.getHomeAddress());
					employeeVO.setOfficeAddress(employee.getOfficeAddress());
	...
	public ApiResponseVO saveEmployee(EmployeeVO employeeVO) {
		...
		try {
			...
			employee.setHomeAddress(employeeVO.getHomeAddress());
			employee.setOfficeAddress(employeeVO.getOfficeAddress());
			...
			if (emp != null) {
				...
				savedEmployeeVO.setHomeAddress(emp.getHomeAddress());
				savedEmployeeVO.setOfficeAddress(emp.getOfficeAddress());
	...
	public ApiResponseVO updateEmployee(EmployeeVO employeeVO) {
		...
		try {
			...
			if (foundEmployee != null) {
				...
				foundEmployee.setHomeAddress(employeeVO.getHomeAddress());
				foundEmployee.setOfficeAddress(employeeVO.getOfficeAddress());
				...
				if (savedEmployee != null) {
					...
					savedEmployeeVO.setHomeAddress(savedEmployee.getHomeAddress());
					savedEmployeeVO.setOfficeAddress(savedEmployee.getOfficeAddress());
					apiResponse.setData(savedEmployeeVO);
					apiResponse.setSuccess(true);
